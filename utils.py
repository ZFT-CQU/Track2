...
def resample(image, voxel_spacing, new_spacing=None, new_shape=None, order=1):
    ...
    ] and np.min(image) == 0 and np.max(image) <= 50 and order == 3:
        warnings.warn(
    ...
    if new_shape is not None:
        ...
        scan_sz_mm = [
            sz * voxel_spacing[idx] for idx, sz in enumerate(image.shape)
        ]
        new_shape = [
    ...
    image = ndi.interpolation.zoom(image,
                                   real_resize_factor,
                                   mode='nearest',
                                   order=order)
...
def poisson_blend(nodule, lung_photo, x0, x1, y0, y1):
    ...
    cv2.imwrite('nodule.jpg', nodule * 255)
    ...
    # convert np to cv2
    cv2.imwrite('CXR-healthy.jpg', im * 255)
    cv2.imwrite('nodule-clip.jpg', obj * 255)
    im2 = cv2.imread('CXR-healthy.jpg')
    obj2 = cv2.imread('nodule-clip.jpg')

    # add gaussian blurring to reduce artefacts
    cv2.imwrite('nodule-mask.jpg', obj2 / 255)
    ...
    mixed_clone2 = cv2.seamlessClone(obj2, im2, mask2, center,
                                     cv2.MIXED_CLONE)
    cv2.imwrite('CXR-diseased.jpg', mixed_clone2)
...
def process_CT_patches(ct_path, seg_path, required_diameter):
    ...
    image_resampled[seg_image_resampled <= np.min(
        seg_image_resampled)] = np.min(image_resampled)
